...
function _renderCMWeek() {
  ...
  let rows = '';
  let sno  = 1;
  // Render the 4 departments in canonical order first, then any other
  // group present (legacy/blank team values) so every devotee stays
  // visible here for assignment — this is where teams get managed.
  const orderedTeams = [
...
async function openChangeCallingBy(devoteeId, devoteeName, team, currentCaller) {
  ...
  try {
    ...
    sel.innerHTML = '<option value="">— Select caller —</option>' +
      users.map(u => {
        const pos = u.position || ((u.role === 'deptCoordinator' || u.role === 'teamAdmin') ? 'Dept Coordinator' : 'Facilitator');
